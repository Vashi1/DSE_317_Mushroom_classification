...
def xgb_kn_classifier_pca(x_tr, x_te, y_tr, y_te):
    clf = XGBClassifier()
    print("XGBoosting Classifier without using any Cross-Validation Technique and PCA Feature Selection")
    clf.fit(x_tr, y_tr)
...
def xgb_kn_classifier_pca_kfold(x_data, y_data,x_tr, x_te, y_tr, y_te):
    originalclass, predictedclass = [], []
    lr_cl_kn_pca = XGBClassifier()
    print("XGBoosting Classifier with K-Fold Cross-Validation Technique(K = 10) and PCA Feature Extraction")
...
def xgb_kn_classifier_pca_stratfold(x_data, y_data, x_tr, x_te, y_tr, y_te):
    originalclass, predictedclass = [], []
    lr_cl_kn_pca_stratf = XGBClassifier()
    print("XGBoosting with Stratified K-Fold Cross-Validation Technique and PCA Feature Extraction Technique")
...
def xgb_kn_classifier_kpca(x_tr, x_te, y_tr, y_te):
    clf = XGBClassifier()
    print("XGBoosting Classifier without using any Cross-Validation Technique and Kernel PCA Feature Selection")
    clf.fit(x_tr, y_tr)
    ypr = clf.predict(x_te)
    print("Classification Report")
    print(classification_report(y_te, ypr))
...
def xgb_kn_classifier_kpca_kfold(x_data, y_data,x_tr, x_te, y_tr, y_te):
    originalclass, predictedclass = [], []
    lr_cl_kn_pca = XGBClassifier()
    print("XGBoosting Classifier with K-Fold Cross-Validation Technique(K = 10) and Kernel PCA Feature Extraction")
    outer_cv = KFold(n_splits=10, shuffle=True, random_state=42)
    pre_scorer_macro = make_scorer(precision_score, average = 'macro')
    pre_scorer_weighted = make_scorer(precision_score, average = 'weighted')
    nested_score_precision_macro = cross_val_score(lr_cl_kn_pca, X=x_data, y=y_data, cv=outer_cv, scoring=pre_scorer_macro)
    ...
    cormat = confusion_matrix(np.array(y_te).argmax(axis = 1), np.array(y_pr).argmax(axis = 1))
    sns.heatmap(cormat, annot=True, fmt = 'g')
    plt.savefig("Implementing XGBoosting Classifier on Data with Dummy Encoding with KNN-Imputation and Kernel PCA Feature Selection with K-Fold Cross Validation.png")
    plt.show()


def xgb_kn_classifier_kpca_stratfold(x_data, y_data, x_tr, x_te, y_tr, y_te):
    originalclass, predictedclass = [], []
    lr_cl_kn_pca_stratf = XGBClassifier()
    print("XGBoosting with Stratified K-Fold Cross-Validation Technique and Kernel PCA Feature Extraction Technique")
    outer_cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    pre_scorer_macro = make_scorer(precision_score, average='macro')
    pre_scorer_weighted = make_scorer(precision_score, average='weighted')
    y_data = np.argmax(y_data, axis=1)
    ...
    sns.heatmap(cormat, annot=True, fmt='g')
    plt.savefig(
        "Implementing XGBoosting Classifier on Data with Dummy Encoding with KNN-Imputation and Kernel PCA Feature Selection with Stratified K-Fold Cross Validation.png")
    plt.show()


def xgb_kn_classifier_trsvd(x_tr, x_te, y_tr, y_te):
    clf = XGBClassifier()
    print("XGBoosting Classifier without using any Cross-Validation Technique and Truncated SVD Feature Selection")
    clf.fit(x_tr, y_tr)
    ypr = clf.predict(x_te)
    print("Classification Report")
    print(classification_report(y_te, ypr))
...
def xgb_kn_classifier_trsvd_kfold(x_data, y_data,x_tr, x_te, y_tr, y_te):
    originalclass, predictedclass = [], []
    lr_cl_kn_pca = XGBClassifier()
    print("XGBoosting Classifier with K-Fold Cross-Validation Technique(K = 10) and Truncated SVD Feature Extraction")
    outer_cv = KFold(n_splits=10, shuffle=True, random_state=42)
    pre_scorer_macro = make_scorer(precision_score, average = 'macro')
    pre_scorer_weighted = make_scorer(precision_score, average = 'weighted')
    nested_score_precision_macro = cross_val_score(lr_cl_kn_pca, X=x_data, y=y_data, cv=outer_cv, scoring=pre_scorer_macro)
    ...
    cormat = confusion_matrix(np.array(y_te).argmax(axis = 1), np.array(y_pr).argmax(axis = 1))
    sns.heatmap(cormat, annot=True, fmt = 'g')
    plt.savefig("Implementing XGBoosting Classifier on Data with Dummy Encoding with KNN-Imputation and Truncated SVD Feature Selection with K-Fold Cross Validation.png")
    plt.show()


def xgb_kn_classifier_trsvd_stratfold(x_data, y_data, x_tr, x_te, y_tr, y_te):
    originalclass, predictedclass = [], []
    lr_cl_kn_pca_stratf = XGBClassifier()
    print("XGBoosting with Stratified K-Fold Cross-Validation Technique and Truncated SVD Feature Extraction Technique")
    outer_cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    pre_scorer_macro = make_scorer(precision_score, average='macro')
    pre_scorer_weighted = make_scorer(precision_score, average='weighted')
    y_data = np.argmax(y_data, axis = 1)
    ...
    cormat = confusion_matrix(np.array(y_te).argmax(axis = 1), np.array(y_pr).argmax(axis = 1))
    sns.heatmap(cormat, annot=True, fmt = 'g')
    plt.savefig("Implementing XGBoosting Classifier on Data with Dummy Encoding with KNN-Imputation and Truncated SVD Feature Selection with Stratified K-Fold Cross Validation.png")
    plt.show()